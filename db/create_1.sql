...
COMMENT ON COLUMN delivery_status.id IS
    'ID';

COMMENT ON COLUMN delivery_status.name IS
    'Наименование';

ALTER TABLE delivery_status ADD CONSTRAINT delivery_status_pk PRIMARY KEY ( id );
...
CREATE TABLE volunteer (
    ...
    volunteer_test_id  INTEGER NOT NULL
);
...
COMMENT ON COLUMN volunteer_test.id IS
    'ID';

COMMENT ON COLUMN volunteer_test.date_pass IS
    'Дата сдачи';

COMMENT ON COLUMN volunteer_test.pass_result IS
    'Результат';

ALTER TABLE volunteer_test ADD CONSTRAINT volunteer_test_pk PRIMARY KEY ( id );
...
ALTER TABLE product_set
    ADD CONSTRAINT product_set_delivery_status_fk FOREIGN KEY ( delivery_status_id )
...
ALTER TABLE product_set_volunteer
    ADD CONSTRAINT prod_set_vounteer_prod_set_fk FOREIGN KEY ( product_set_id )
        REFERENCES product_set ( id );

--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE product_set_volunteer
    ADD CONSTRAINT prod_set_volun_volun_fk FOREIGN KEY ( volunteer_id )
...
ALTER TABLE volunteer
    ADD CONSTRAINT volunteer_volunteer_test_fk FOREIGN KEY ( volunteer_test_id )
        REFERENCES volunteer_test ( id );
